...
class DemoSchemasTest {
    ...
    void theSortOrderTravelsWithTheSchema() {
        // ascham.sort_key declares (sym, ts), so the cold tier needs no separate configuration to
    ...
    void varlenBudgetsScaleWithTheBatchSize() {
        // ascham.varlen_bytes is a per-batch budget for the whole column, so it must track batch_rows;
