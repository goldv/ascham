...
/**
 * Strict, total validation of an Arrow schema plus its {@code ascham.*} metadata. Collects
 ...
 *   <li>{@code ascham.table} present and non-empty; {@code ascham.schema_version} a valid integer</li>
 *   <li>{@code ascham.batch_rows}, if present, a positive integer</li>
 *   <li>{@code ascham.time_column} present and naming a {@code Timestamp} column</li>
 *   <li>{@code ascham.stats_column}, if present, naming a fixed-width integer column</li>
 *   <li>{@code ascham.varlen_bytes} present and positive for every {@code Utf8}/{@code Binary}
 *       column, and absent on all others</li>
 *   <li>{@code ascham.sort_key}, if present, a non-negative integer, unique across columns</li>
 *   <li>{@code ascham.ref} only on signed {@code Int32} columns</li>
 *   <li>no unknown {@code ascham.*} keys at either level</li>
 * </ul>
 */
