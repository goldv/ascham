...
>   parquet file (~115 MB segments × N), replacing reliance on engine write behaviour. Sort order
>   is now *declared* in Iceberg metadata at create (DuckDB could not) *and* produced physically by
>   a native index sort: sort-key columns are extracted from the zero-copy mmap roots, an index
>   permutation is sorted, and rows stream out in order — memory bounded by one group's keys, no
>   external-sort spill machinery.
> - **Automatic reclaim was removed** from the pass (`SegmentReclaimer` deleted); a standalone
>   utility will consume the snapshot-summary provenance, and must do so *before* expiring
>   snapshots.
> - **Nanosecond timestamps** are stored as **unzoned** `timestamp_ns` (UTC by convention, exactly
>   like the DuckDB roller's `TIMESTAMP_NS`): DuckDB has no zoned nanosecond type, and the query
>   surface must be able to read the data.
>
> §§4–5 below are kept as written for the R1–R5 record; read them as the history of the protocol,
> whose invariants (I1–I3, the freeze check, day-alignment verification, ascending abort) carried
> over unchanged.

## 1. Decisions

- **Storage**: Parquet under an **Iceberg catalog**. v1 dev stack is Lakekeeper + MinIO
  (docker-compose, §9); the interface is the standard REST catalog spec, so Polaris/Glue/S3-Tables
  are a config change later. *(R5.5: a local path destination is also supported — serverless
  HadoopCatalog straight to disk; see the revision note.)*
- **Roll trigger**: **EOD roll of prior day(s)**, kdb-style — pull-based, scheduled + on-startup +
  retry; not event-driven off rotation.
- **Roll writer** *(superseded at R5.5 — see revision note)*: v1 embedded DuckDB; the "kept
  swappable" pure-Java Iceberg writer is now the implementation, behind the same `RollExecutor`
  seam.
...
*(R5.5: the recommendation below was implemented at R4–R5 and then flipped — option (b), the
native Java writer, is now the implementation. What changed the balance in practice: the
extension-version lockstep between duckdb_jdbc and the locally built arena extension on the write
path, the DDL gaps (no declarable sort order, the quoted-`'format-version'` trap), and the S3
secret/vending quirks. The (b) costs feared below turned out smaller: hadoop shims are a shaded
two-artifact pair, and the "hand-rolled external sort" became a bounded-memory index sort over the
mmap roots — no spill machinery at all. The §4 protocol was writer-agnostic as designed;
`RollExecutor` swapped implementations without touching the coordination logic.)*

**Original recommendation (R1–R5): (a) `:cold` embeds DuckDB** — Java purely orchestrates
(discovery, freeze checks, verification, scheduling, retries, unlink); DuckDB moves the bytes.
...
arena branch of the union so the two union branches type-match positionally. *(R5.5: on the write
side this table is now implemented by `IcebergTypes` — the Iceberg column carries the same types,
...
- *(R5.5)* **The correctness suite is hermetic**: `LocalRollTest` runs the full roll — real
...
- **`:cold` integration tests** *(historical R4–R5 shape)*: write days with `RotatingWriter`,
  ...
  three recovery branches (§3.3), including kill-between-commits convergence with a dupe-detector
...
1. **Iceberg-extension write maturity** (V3 ns writes, partition/sort DDL, multi-table txn
   ...
   fallback; recovery branches make non-atomic commits safe. *(R5.5: retired — the write path no
   longer goes through the extension. The residual risk moved to iceberg-java's `timestamp_ns`
   generics, which the `IcebergNanoSpike` + `LocalRollTest` nano-fidelity tests pin, and to
   read-side compatibility, which `RestCatalogIT`'s DuckDB read-back pins.)*
...
5. **Full-day sort memory** — *(R5.5)* the index sort is bounded by one file group's sort-key
   columns (8 bytes/row/key + varlen distinct values), and the group size is capped by
   `segmentsPerFile` × segment capacity; there is no whole-day in-memory materialisation and no
   spill machinery to configure.
