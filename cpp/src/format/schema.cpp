...
void read_table_metadata(const char* metadata, TableSchema& out) {
    ...
    while (ArrowMetadataReaderRead(&reader, &key, &value) == NANOARROW_OK) {
        ...
        if (k == fmt::META_TIME_COLUMN) {
            out.time_column = sv_to_string(value);
        } else if (k == fmt::META_STATS_COLUMN) {
...
ColumnType map_column(const ArrowSchema* field, ArrowError& error) {
    ...
    // The view's type is the index type for dictionary-encoded fields, which would silently
    // misread codes as values; the profile rejects dictionaries outright (as the writer does).
    if (field->dictionary != nullptr) {
        throw FormatError("column '" + col.name + "': dictionary encoding is outside the v1 profile");
    }
    switch (view.type) {
    ...
    case NANOARROW_TYPE_TIME64:
        // A microsecond Time64 read as nanoseconds would be off by 1000x — reject, as the writer does.
        if (view.time_unit != NANOARROW_TIME_UNIT_NANO) {
            throw FormatError("column '" + col.name + "': only Time64(ns) is in the v1 profile");
        }
        col.type = LogicalType::TIME64_NS;
        break;
    case NANOARROW_TYPE_TIMESTAMP:
        col.type = LogicalType::TIMESTAMP;
        if (view.time_unit == NANOARROW_TIME_UNIT_NANO) {
            col.timestamp_unit = TimestampUnit::NANO;
        } else if (view.time_unit == NANOARROW_TIME_UNIT_MICRO) {
            col.timestamp_unit = TimestampUnit::MICRO;
        } else {
            throw FormatError("column '" + col.name + "': only Timestamp(ns|us) is in the v1 profile");
        }
        if (view.timezone) {
