...
MappedFile MappedFile::open(const std::string& path) {
    ...
    if (size == 0) {
        // mmap rejects a zero length with EINVAL, which reads as an internal error rather than the
        // truncated file it actually is.
        ::close(fd);
        throw std::system_error(EINVAL, std::generic_category(), "empty segment file " + path);
    }
