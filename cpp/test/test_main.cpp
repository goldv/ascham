...
TEST(table_dir_lists_segments_oldest_first) {
    ...
    for (const char* name : {"20260729.0.ascham", "20260728.1.ascham", "20260728.0.ascham",
                             "20260728.0.ascham.tmp.1.1", "notasegment.txt"}) {
...
TEST(table_dir_orders_sub_day_segments_by_start_then_cycle) {
    namespace fs = std::filesystem;
    fs::path dir = fs::temp_directory_path() / "arena_table_dir_subday_test";
    fs::remove_all(dir);
    fs::create_directories(dir);
    for (const char* name : {"20260804.0400.240m.1.ascham", "20260804.0.ascham",
                             "20260804.0400.240m.0.ascham", "20260803.2000.240m.0.ascham",
                             "20260804.0000.240m.0.ascham", "20260804.0400.240m.1.ascham.tmp.5.7",
                             "20260804.ascham", "notasegment.txt"}) {
        std::ofstream(dir / name).put('x');
    }
    auto segs = arena::list_segments(dir.string());
    CHECK_EQ(segs.size(), 5u);
    // Mirrors Java SegmentDirectoryTest: (start, cycle minutes, seq) — a 240m segment starting at
    // midnight sorts before the daily segment with the same start.
    CHECK_EQ(fs::path(segs[0].path).filename().string(), "20260803.2000.240m.0.ascham");
    CHECK_EQ(fs::path(segs[1].path).filename().string(), "20260804.0000.240m.0.ascham");
    CHECK_EQ(fs::path(segs[2].path).filename().string(), "20260804.0.ascham");
    CHECK_EQ(fs::path(segs[3].path).filename().string(), "20260804.0400.240m.0.ascham");
    CHECK_EQ(fs::path(segs[4].path).filename().string(), "20260804.0400.240m.1.ascham");
    CHECK_EQ(segs[0].start_hhmm, 2000);
    CHECK_EQ(segs[0].cycle_minutes, 240);
    CHECK_EQ(segs[2].start_hhmm, 0);
    CHECK_EQ(segs[2].cycle_minutes, 1440);
    CHECK_EQ(segs[4].sequence, 1);
    fs::remove_all(dir);
}
