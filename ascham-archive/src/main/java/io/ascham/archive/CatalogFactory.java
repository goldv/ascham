...
final class CatalogFactory {
    ...
    static Catalog open(ArchiveConfig config) {
        String destination = config.destination();
        if (destination.startsWith("http://") || destination.startsWith("https://")) {
            return rest(config);
        }
        if (destination.startsWith("s3://")) {
            throw new ArchiveException("object-store warehouses need a REST catalog: point destination "
                    + "at the catalog endpoint (http(s)://...) instead of " + destination);
        }
        return hadoop(config);
    }

    private static Catalog rest(ArchiveConfig config) {
        ...
        ArchiveConfig.S3Credentials s3 = config.s3();
        if (s3 != null) {
    ...
    private static Catalog hadoop(ArchiveConfig config) {
        String destination = config.destination();
