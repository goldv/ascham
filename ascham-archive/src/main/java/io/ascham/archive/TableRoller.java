...
public final class TableRoller {
    ...
    public TableRoller(ArchiveConfig config, RollExecutor executor) {
        this.config = config;
     ...
     * @throws ArchiveException                             if the store rejects a roll (aborts the run)
     */
    ...
    private SegmentDirectory openExistingTableDir(String table) {
        ...
        if (!java.nio.file.Files.isDirectory(tableDir)) {
            throw new ArchiveException("no arena table directory at " + tableDir);
        }
