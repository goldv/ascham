...
public final class RollService {
    ...
    public RollService(ArchiveConfig config, RollExecutor executor, long arenaBytesAlertThreshold) {
    ...
    private static boolean holdsSegments(Path tableDir) {
        try (Stream<Path> files = Files.list(tableDir)) {
            return files.anyMatch(p -> p.getFileName().toString().endsWith(".ascham"));
        } catch (IOException e) {
            return false;
        }
    }
    ...
    public long arenaBytes() {
        ...
        try (Stream<Path> walk = Files.walk(base)) {
            return walk.filter(Files::isRegularFile)
                    .filter(p -> p.getFileName().toString().endsWith(".ascham"))
                    .mapToLong(p -> {
                        try {
                            return Files.size(p);
                        } catch (IOException e) {
